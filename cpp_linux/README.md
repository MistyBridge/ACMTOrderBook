...
引擎内部价格统一为 **×10⁶**(逐笔 ×10⁴/×10³ 升档、快照 ×10⁶, 全 int64),换算在 `.so` 内完成:

- 内部价格统一到官方快照的 ×10⁶,与 `cpp v2/behave` 单一源码树一致;数量/金额为交易所原生精度。
- 价格必须 int64:×10⁶ 下 int32 上限仅 2147.48 元,高价股(如贵州茅台)会超出。
- 成交额累加为 `price × qty` 的 int64 乘积,再落在交易所金额精度;对境内单笔远低于
  int64 上限,无需 `__int128`(与 `cpp v2` 一致)。

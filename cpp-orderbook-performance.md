...
- Compiler: MSVC 2022 (Visual Studio 17 2022 Build Tools, v143)
- OS: Windows
...
> **统计口径说明（与 README 一致）**
>
> - **吞吐 = T1 系统端到端吞吐**：总消息数 ÷ 整段墙钟时间（含文件读/mmap、解析、生产者、队列、双线程、引擎），反映全部架构与数据路径优化。
> - **延迟 = L1 单事件处理延迟**：每条真实消息 `onMsg()` 的处理耗时，逐条全量计时，报告 `p50/p99/p99.9/pmax`。
> - **计时与百分位约定**：一律用单调时钟（Windows `QueryPerformanceCounter`，Linux `steady_clock`，Python `time.perf_counter_ns()`）；百分位索引统一 `floor(p×(N-1))`。
...
| CPP v2.1 | 多线程 SPSC MSVC | 212,817 msg/s | +229.4% | HybridLevelBook + MSVC 优化 |
...
**迁移到 MSVC 2022 后，此问题不再存在。** MSVC 的优化器更稳定，支持全程序优化 (`/GL` + `/LTCG`)。
...
- **状态**: 已完成
- **标签**: `v2.1` (`40bcac2`)
- **编译选项**: `/O2 /GL /arch:AVX2 /utf-8 /std:c++17` (MSVC 2022)
- **目录**: `cpp v2/`
- **性能**: 212,817 msg/s (+122.9% vs v2, +229.4% vs v1)
...
# 方法 1: CMake + MSBuild (推荐)
cmake -B build -G "Visual Studio 17 2022" -A x64
cmake --build build --config Release --parallel 8
cp build/Release/orderbook_v2.exe orderbook_v2.1.exe
...
| `/utf-8` | 源文件 UTF-8 编码 (避免中文注释 C4819) |
| `/std:c++17` | C++17 标准 |
| `/DNDEBUG` | 禁用 assert |
| `/LTCG` | 链接时代码生成 |
...
### 性能指标 (单次重放, 233,875 msgs)
| 指标 | 数值 | vs v2 | vs v1 |
|------|------|-------|-------|
| 总耗时 | 1.099 s | -59.9% | -69.6% |
| 吞吐量 | 212,817 msg/s | +122.9% | +229.4% |
| p50 | 0.3 us | 不变 | - |
| p99 | 162.0 us | +126.6% | - |
| p99.9 | 2299.4 us | - | - |
| pmax | 2640.0 us | - | - |
...
| p99 | 71.5 us | 162.0 us | +126.6% |
| p99.9 | 521.8 us | 2299.4 us | - |
| pmax | 957.3 us | 2640.0 us | - |
...
| 2026-06-13 | v2.1 | HybridLevelBook + prefetch, 164K msg/s (MinGW) |
| 2026-06-13 | - | Dashboard P50/P99/Pmax + replay |
| 2026-06-14 | v2.1 | 迁移到 MSVC 2022, 吞吐量提升至 213K msg/s (+30%) |
| 2026-06-14 | - | 修复仪表盘重放次数速度计算 bug |

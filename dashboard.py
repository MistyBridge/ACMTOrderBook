...
class Dashboard(tk.Tk):
    ...
    def _build_header(self):
        ...
        tk.Label(h, text=f"  000001 | 2022-04-22 | {_format_number(BASE_MSG_COUNT)} msgs",
    ...
    def _start_run(self, r, key):
        ...
        r.result_lines = []
        r.msg_count = 0
        r.replay_count = replay_count
        r._speed_samples = []
        r._last_calc_time = 0.0
        r._last_calc_count = 0
        r.bar["value"] = 0
        ...
        total_msgs = BASE_MSG_COUNT * replay_count
        self._log(f"\n{'='*60}")
        self._log(f"  [{r.name}] start  {time.strftime('%H:%M:%S')}")
        self._log(f"  CMD: {' '.join(cmd)}")
        self._log(f"  Total: {_format_number(total_msgs)} msgs ({replay_count} replays)")
        self._log(f"{'='*60}")
    ...
    def _parse_line(self, r, line):
        # 计算总消息数（考虑重放次数）
        total_msgs = BASE_MSG_COUNT * r.replay_count
        ...
        if m:
            r.msg_count = int(m.group(1))
            r.progress = min(r.msg_count / total_msgs * 100, 99.9)

        # cpp progress (v2: "produced 50000 msgs..." or "consumed 50000 msgs...")
        m = re.search(r"(?:produced|consumed)\s+(\d+)\s+msgs", line)
        if m:
            r.msg_count = int(m.group(1))
            r.progress = min(r.msg_count / total_msgs * 100, 99.9)
    ...
    def _calc_realtime_speed(self, r) -> int:
        """
        计算实时速度（msg/s），使用滑动窗口平滑
        修复：考虑重放次数，基于实际消息计数计算
        """
        now = time.time()
        ...
        # 方法1：基于解析到的消息计数（最准确）
        if r.msg_count > 0:
            raw_speed = r.msg_count / elapsed
        else:
            # 方法2：基于进度百分比估算（备用）
            total_msgs = BASE_MSG_COUNT * r.replay_count
            raw_speed = total_msgs * r.progress / 100 / elapsed

        # 滑动窗口平滑（保留最近5个采样）
        r._speed_samples.append(raw_speed)
        if len(r._speed_samples) > 5:
            r._speed_samples.pop(0)

        # 返回平均值
        return int(sum(r._speed_samples) / len(r._speed_samples))
    ...
    def _tick(self):
        for r in self.runners.values():
            ...
            if r.running:
                ...
                if el > 0.5 and r.progress > 0.1:
                    # 计算实时速度
                    sp = self._calc_realtime_speed(r)

                    # 计算ETA
                    if r.progress > 0:
                        total_est_time = el / (r.progress / 100)
                        rm = max(0, total_est_time - el)
                    else:
                        rm = 0

                    r.spd.config(text=f"{_format_number(sp)} msg/s │ ETA {rm:.0f}s")
                    r.stats["Time"].config(text=f"{el:.1f}s")
                    r.stats["Msg/s"].config(text=_format_number(sp))
                elif el > 0:
